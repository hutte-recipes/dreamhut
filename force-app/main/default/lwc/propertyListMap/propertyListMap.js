...
export default class PropertyListMap extends LightningElement {
    ...
    displayProperties() {
        ...
        const markers = this.properties.map((property) => {
            const latLng = [property.Hutte__Location__Latitude__s, property.Hutte__Location__Longitude__s];
    ...
    getTooltipMarkup(property) {
        return `<div class="tooltip-picture" style="background-image:url(${property.Hutte__Thumbnail__c})">  
  <div class="lower-third">
    <h1>${property.Name}</h1>
    <p>Beds: ${property.Hutte__Beds__c} - Baths: ${property.Hutte__Baths__c}</p>
  </div>
</div>`;
    }
}